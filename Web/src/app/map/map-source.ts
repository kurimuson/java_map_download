...
export class MapSource {
	...
	public putAddedLayers(addedLayers: Array<{ name: string, url: string, type: string }>): void {
		for (let i = 0; i < addedLayers.length; i++) {
			if (!this.layers.has(addedLayers[i].name)) {
				this.layers.set(addedLayers[i].name, [
					{
						name: addedLayers[i].name, type: 'XYZ_URL', support: true,
						source: new ol.source.XYZ({ url: addedLayers[i].url }),
